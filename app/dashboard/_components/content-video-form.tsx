...
interface ContentVideoFormProps {
  initialData: {
...
export const ContentVideoForm = ({
  ...
  const baseUrl = `/api/courses/${courseId}/units/${unitId}/content/${contentId}`;
  ...
  const onSubmitUpload = async (url: string) => {
    try {
      ...
      console.error("[CONTENT_VIDEO]", error);
  ...
  const onSubmitYouTube = async () => {
    ...
    try {
      ...
      console.error("[CONTENT_YOUTUBE]", error);
  ...
  const videoPlayerKey = useMemo(() => {
    if (videoData.videoType === "YOUTUBE" && videoData.youtubeVideoId) {
      return `youtube-${contentId}-${videoData.youtubeVideoId}`;
    }
    if (videoData.videoUrl) {
      return `upload-${contentId}-${videoData.videoUrl}`;
    }
    return null;
  }, [contentId, videoData.videoType, videoData.youtubeVideoId, videoData.videoUrl]);

  const hasVideo =
    videoData.videoUrl ||
    (videoData.videoType === "YOUTUBE" && videoData.youtubeVideoId);
        ...
        <div className="relative aspect-video mt-2">
          {hasVideo && videoPlayerKey ? (
            <PlyrVideoPlayer
              key={videoPlayerKey}
              videoUrl={
                videoData.videoType === "UPLOAD" ? videoData.videoUrl ?? undefined : undefined
              }
              youtubeVideoId={
                videoData.videoType === "YOUTUBE"
                  ? videoData.youtubeVideoId ?? undefined
                  : undefined
              }
            ...
            <TabsContent value="upload" className="mt-4">
              <div className="space-y-4">
                ...
                <FileUpload
                  ...
                  onChange={(res) => {
                    if (res?.url) onSubmitUpload(res.url);
                  }}
                />
              </div>
            </TabsContent>
